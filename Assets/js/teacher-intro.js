    ...
    if (teacherDataStr) {
        ...
        if (teacherDetailContainer) {
            ...
            const booksHTML = teacherBooks.map(book => {
                // Add line breaks to titles based on patterns
                let displayTitle = book.title;
                
                // Skip line breaks for "ダイジュ先生", "上野", and "伊藤" titles
                const isDaijuTitle = displayTitle.includes('ダイジュ先生');
                const isUenoTitle = displayTitle.includes('上野のたった');
                const isItoTitle = displayTitle.includes('伊藤のたった');
                
                // Special pattern for "杉本のたった5時間で英検®準2級プラス 総合対策"
                if (displayTitle.includes('準2級プラス 総合対策')) {
                    displayTitle = displayTitle.replace(/(準2級プラス) (総合)(対策)/, '$1<br>$2<br> $3');
                } else if (!isDaijuTitle && !isUenoTitle && !isItoTitle) {
                    // Pattern 1: "たったX時間で" followed by subject
                    if (displayTitle.includes('時間で')) {
                        displayTitle = displayTitle.replace(/(時間で)/, '$1<br>');
                    }
                    
                    // Pattern 2: "【1】" or "【2】" followed by text
                    if (displayTitle.includes('【1】')) {
                        displayTitle = displayTitle.replace(/(【1】)/, '$1<br>');
                    } else if (displayTitle.includes('【2】')) {
                        displayTitle = displayTitle.replace(/(【2】)/, '$1<br>');
                    }
                    
                    // Pattern 3: "良問特訓" followed by subject (without 【1】 or 【2】)
                    if (displayTitle.includes('良問特訓') && !displayTitle.includes('【')) {
                        displayTitle = displayTitle.replace(/(良問特訓)/, '$1<br>');
                    }
                }
                ...
                <div class="teacher-detail-book-item">
                    <div class="teacher-detail-book-top">
                        ...
                        <div class="teacher-detail-book-info">
                            <p class="teacher-detail-book-title">${displayTitle}</p>
                            ...
                            <div class="teacher-detail-book-bottom">
                                <a href="#" class="teacher-detail-book-button" data-book-id="${book.id}">詳細はこちら</a>
                            </div>
                        </div>
                    </div>
                </div>
